...
import { CitaVisualizacionAsistenteComponent } from '../cita-visualizacion-asistente/cita-visualizacion-asistente.component';
import { CitaRegistroAsistenteComponent } from '../cita-registro-asistente/cita-registro-asistente.component';
import { CitaRegistroConfirmacionAsistenteComponent } from '../cita-registro-confirmacion-asistente/cita-registro-confirmacion-asistente.component';
...
@Component({
  selector: 'app-citas-asistente',
  changeDetection: ChangeDetectionStrategy.OnPush,
  templateUrl: './citas-asistente.component.html',
  styleUrls: ['./citas-asistente.component.scss'],
  providers: [
    {
...
export class CitasAsistenteComponent implements OnInit {

  displayedColumns: string[] = ['nombre', 'cedula', 'fechahora', 'acciones'];
  ...
  editarCita(cita: Cita): void {
    const referenciaDialogo = this.dialog.open(CitasAsistenteComponent, {
  ...
  iniciarCita(cita: Cita): void {
    const referenciaDialogo = this.dialog.open(CitaVisualizacionAsistenteComponent, {
      data: { idCita: cita.id_cita }, minWidth: 400
    });
  ...
  registrarCita() {
    const referenciaDialogo = this.dialog.open(CitaRegistroAsistenteComponent, {
      minWidth: 400
    });
    ...
    referenciaDialogo.afterClosed().subscribe(result => {
      ...
      const referenciaDialogoNueva = this.dialog.open(CitaRegistroConfirmacionAsistenteComponent, {
